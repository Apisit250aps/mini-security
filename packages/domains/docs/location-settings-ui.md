...
- Sidebar กลุ่ม **ข้อมูลบริษัท → Locations** เปิด `/company/locations` แสดงสถานที่ทั้งหมดของบริษัทที่เลือก รวมสถานที่ปิดใช้ พร้อมสาขา ที่อยู่ พิกัด รัศมีหน่วยเมตร และสถานะหลักของสาขา หน้านี้ใช้ดูข้อมูล
...
- เทสต์เดิม `attendance-schedules.test.ts` ยังมี 2 failures ที่ยืนยันซ้ำกับโค้ดก่อนการแก้ไข: fixture manual check-in ไม่มี `companyId` และ mock leave quota ไม่มี `lockByMemberTypeAndYear`
