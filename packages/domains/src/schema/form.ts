...
export const FormReviewActionValues = [
  'PASS',
  'NEEDS_CHANGES',
  'APPROVE',
  'RETURN',
...
export const formPlanSchema = baseFormPlanSchema.refine(
  (data) =>
    (data.scheduleKind === 'RECURRING' && data.scheduleConfig !== null) ||
    (data.scheduleKind === 'EXPLICIT' && data.scheduleConfig === null),
...
export const updateFormPlanSchema = baseFormPlanSchema
  .partial()
  .omit({ id: true, createdAt: true, updatedAt: true });
...
export const createFormPlanPeriodSchema = baseFormPlanPeriodSchema
  .omit({ id: true, createdAt: true })
  .refine((data) => data.dueAt > data.opensAt, { message: 'dueAt > opensAt' });
...
export const createFormOccurrenceSchema = baseFormOccurrenceSchema.omit({
  id: true,
  createdAt: true,
});
export const updateFormOccurrenceSchema = baseFormOccurrenceSchema
  .partial()
  .omit({ id: true, createdAt: true }); // specifically for cancel
...
export const updateFormAssignmentSchema = baseFormAssignmentSchema
  .partial()
  .omit({ id: true, createdAt: true }); // specifically for cancel
...
export const createFormSubmissionSchema = baseFormSubmissionSchema.omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export const updateFormSubmissionSchema = baseFormSubmissionSchema
  .partial()
  .omit({ id: true, createdAt: true, updatedAt: true });
...
export const createFormSubmissionContributorSchema =
  formSubmissionContributorSchema.omit({ id: true, createdAt: true });
...
export const formReviewEntrySchema = baseFormReviewEntrySchema
  ...
  .refine(
    (data) =>
      !['NEEDS_CHANGES', 'RETURN'].includes(data.action) ||
  ...
  items: z
    ...
    .refine((items) => {
      ...
      return (
        ...
        items.every((i) => i.sortOrder < items.length)
      );
    }),
});
