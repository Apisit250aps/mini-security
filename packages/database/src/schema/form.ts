...
export const formMissedPolicyEnum = pgEnum('form_missed_policy', [
  'SKIP',
  'CATCH_UP',
]);
export const formReviewActionEnum = pgEnum('form_review_action', [
  'PASS',
  'NEEDS_CHANGES',
  'APPROVE',
  'RETURN',
...
export const formVersion = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('form_one_draft_version')
...
export const formPlan = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`effective_until IS NULL OR (effective_from IS NOT NULL AND effective_until > effective_from)`,
    ),
    check(
      'form_plan_no_self_check',
      sql`supersedes_plan_id IS NULL OR supersedes_plan_id <> id`,
    ),
    check(
      'form_plan_closed_actor_check',
...
export const formPlanTarget = pgTable(
  'form_plan_target',
  {
    id: primaryKeyUuid7('id'),
    companyId: uuid('company_id')
...
export const formPlanPeriod = pgTable(
  'form_plan_period',
  {
    id: primaryKeyUuid7('id'),
    companyId: uuid('company_id')
...
export const formOccurrence = pgTable(
  'form_occurrence',
  {
    id: primaryKeyUuid7('id'),
    companyId: uuid('company_id')
  ...
  (table) => [
    ...
    unique('form_occurrence_plan_period_unique').on(
      table.planId,
      table.periodId,
    ),
...
export const formAssignment = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('form_active_role_assignment')
    ...
    check(
      ...
      sql`replaces_assignment_id IS NULL OR replaces_assignment_id <> id`,
    ),
    check(
      'form_assignment_cancel_check',
      sql`(cancelled_at IS NULL AND cancelled_by IS NULL AND cancel_reason IS NULL) OR (cancelled_at IS NOT NULL AND cancelled_by IS NOT NULL AND cancel_reason IS NOT NULL AND length(trim(cancel_reason)) > 0)`,
    ),
    foreignKey({
...
export const formReviewEntry = pgTable(
  'form_review_entry',
  {
    id: primaryKeyUuid7('id'),
    companyId: uuid('company_id')
  ...
  (table) => [
    ...
    check(
      ...
      sql`action NOT IN ('NEEDS_CHANGES', 'RETURN') OR (note IS NOT NULL AND length(trim(note)) > 0)`,
    ),
    check(
      'form_review_no_self_check',
