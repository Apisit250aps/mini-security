...
`activeOrganizationId` through `ISecurityContext`. Only trusted server code may build
...
access to another organization. Extractors select the target organization only; they cannot
...
loads current actions for the session's active organization once per request. This
...
the session on every request. Organization switching writes the new organization and its
permissions together. Session refresh must not reset the selected organization.
...
manage an OWNER or act without organization membership. OWNER identity uses
`roleType`, not a mutable display name. Non-admins require active membership in
the target organization, cannot remove an OWNER, change its role, or deactivate it.
Member user/organization reassignment and cross-organization role/site assignments remain
invalid for admins too. Site deletion retains its last-site and assigned-member
invariants.
...
async execute(context: IRemoveOrganizationMemberContext, existing?: OrganizationMember): Promise<void> {
...
checks its stored organization, and passes that exact object to the method. The
resource is loaded once per invocation, including for admins. Missing resources
raise `NotFoundError`; caller-provided organization IDs cannot override stored scope.
...
Member update/removal and site read/update/removal use this pattern. Apply it
